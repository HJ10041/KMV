...
OPEN(3, FILE = trim(OutputDir)//'SteadyStateEqumIteration.txt', STATUS='unknown', POSITION='append')

converged = .false.
neqmiter = 1
IF(.not. calibrating) lstepKN = stepequmss
IF(calibrating) lstepKN = 0.2*stepequmss
ldiffKN = 1.0

! ===== BLOCK 1 START: Main check for model type =====
...
ELSE IF(OneAssetNoCapital==1) THEN
    ...
    ! ===== BLOCK 2 START: Nested check for profit distribution =====
...
! ===== BLOCK 3 START: Final check for government bonds =====
